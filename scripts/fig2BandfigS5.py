...
###################FFT######################################
fft = np.fft.fft(V)
freq = np.fft.fftfreq(len(V), d=step)

# plt.scatter(freq, np.abs(fft))
# plt.title("Spectrum")   #[0.25, 0.30, 0.35, 0.40, 0.45] for robustness analysis
cutoff = 0.45 * max(freq)
...
Vq_fft = fft_lowpass(V, cutoff, step)
Iq_fft = fft_lowpass(I, cutoff, step)
sigma_Vq_fft = fft_lowpass(V_sem, cutoff, step)
sigma_Iq_fft = fft_lowpass(I_sem, cutoff, step)
...
# raw（参考）
plt.plot(tlist2[:len(V)], V, 'k-', alpha=0.3,linewidth=3, label='Raw Signal')

plt.plot(tlist2[:len(V1)], V1, 'k-', linewidth=3, label='Simulation')
plt.xlabel(r'Time (us)', fontname='Arial',
              fontweight='bold',
              fontstyle='normal',
              labelpad=3,
              color='black',fontsize=24)
plt.ylabel(r'$\mathbf{V_q}$', fontweight='bold', fontname='Arial', labelpad=3,fontsize=24, color='black')
# plt.ylabel(r'$V_q$', fontname='Arial',
#               fontweight='bold',
#               fontstyle='normal',
#               labelpad=3,
#               color='black',fontsize=24)
plt.plot(tlist2[:len(Vq_fft)], Vq_fft, 'b--', linewidth=3, label='FFT')
plt.legend()
plt.show()
...
def create_enhanced_iv_plot(I,V, I_std, V_std, I1, V1, save_path='I-V_enhanced.svg'):
...
if __name__ == "__main__":
    ...
    fig, ax = create_enhanced_iv_plot(Iq_fft, Vq_fft, sigma_Iq_fft, sigma_Vq_fft, I1, V1)
