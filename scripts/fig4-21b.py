...
v_a = []
signal_a = []
J2= [(2*1.6*J1_a**2*p1_a[0]*(Gamma**2+4*Delta**2)/((1-p0_a[0]-p1_a[0])*Gamma**2))**0.5]
for i in range(measure_times - 1):
    ...
    J2.append((2*1.6*J1_a**2*p1_a[i+1]*(Gamma**2+4*Delta**2)/((1-p0_a[i+1]-p1_a[i+1])*Gamma**2))**0.5)
    
    states = result1.states[-1]

# ====================== 读取实验数据并实时拟合 ======================
# 1) 读取 I_high 波形实验数据
time, I_exp, I_sem = exp_waveform_data("I_high_waveform.csv")

# 2) 读取布居数实验数据
time_pop, pop_exp, pop_err = exp_population_data("High High data.csv")
...
# 在放大子图中绘制
ax_inset.plot(window_t, window_va, color="#0BD40B", linewidth=4, alpha=0.95)
...
# 计算数据的y值范围，从0开始
y_min = 0
y_max = max(window_va) * 3  # 增加10%的余量
...
# HERE = os.path.dirname(os.path.abspath(__file__))
# save_path = os.path.join(HERE, "11_voltage.svg")
