...
# 波形实验数据
def exp_waveform_data(filename="I_low_waveform.csv"):
    path = DATA / filename
    return read_three_column_csv(path)

# 布居数实验数据
def exp_population_data(filename="Low High data.csv"):
    path = DATA / filename
    return read_three_column_csv(path)
...
def build_Hamiltonian(J1, J2):
    return J1 * H_structure1 + J2 * H_structure2+H_offset
...
J1_a = (3 * (Gamma) * 0.005 / 8) ** 0.5
J2_a = [0]
J1_high=(3 * (Gamma) * 0.0072 / 8) ** 0.5
sampling_factor = 4
high_res_t = []
v_a = []
signal_a = []
J2= [(2*1.6*J1_high**2*p1_a[0]*(Gamma**2+4*Delta**2)/((1-p0_a[0]-p1_a[0])*Gamma**2))**0.5]
for i in range(measure_times - 1):
    t_measure = np.array([0.25 * dt, 0.5 * dt, 0.75 * dt, dt])

    H2 = build_Hamiltonian(J1_a, J2[i])
    ...
    J2.append((2*1.6*J1_high**2*p1_a[i+1]*(Gamma**2+4*Delta**2)/((1-p0_a[i+1]-p1_a[i+1])*Gamma**2))**0.5)
    
    states = result1.states[-1]

# ====================== 读取实验数据并实时拟合 ======================
# 1) 读取 I_high 波形实验数据
time, I_exp, I_sem = exp_waveform_data("I_low_waveform.csv")

# 2) 读取布居数实验数据
time_pop, pop_exp, pop_err = exp_population_data("Low High data.csv")
...
line = ax_main.plot(high_res_t[0:800], v_a[0:800], 
                    color="#070BEB",
                    linewidth=4,
                    label='Low γ , High G ', 
                    alpha=0.95, 
...
ax_main.errorbar(time, v_exp, yerr=v_err, 
...
# 在放大子图中绘制
ax_inset.plot(window_t, window_va, color="#070BEB", linewidth=4, alpha=0.95)

ax_inset.axhline(y=-bias, color="#EE0B0B", linestyle='--', linewidth=3)
ax_inset.errorbar(time, v_exp, yerr=v_err, 
            fmt='o', color="#EE053F", markersize=8,  markeredgecolor="#8A1830",markeredgewidth=2, 
            capsize=6, capthick=2, elinewidth=1,zorder=4,
            )
...
# HERE = os.path.dirname(os.path.abspath(__file__))
# save_path = os.path.join(HERE, "01_voltage.svg")
