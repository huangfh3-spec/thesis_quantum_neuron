...
# ====================== 读取实验数据并实时拟合 ======================
# 1) 读取 I_high 波形实验数据
time, I_high, I_sem = exp_waveform_data("I_low_waveform.csv")

# 2) 读取布居数实验数据
time_pop, pop_exp, pop_err = exp_population_data("Low Low data.csv")
...
line = ax_main.plot(high_res_t[0:320], v_a[0:320], 
                    color="#B709FC",
                    linewidth=4,
                    label='Low γ ,Low G', 
                    alpha=0.95, 
...
# 在放大子图中绘制
ax_inset.plot(window_t, window_va, color="#B709FC", linewidth=4, alpha=0.95)
...
# HERE = os.path.dirname(os.path.abspath(__file__))
# save_path = os.path.join(HERE, "00_voltage.svg")
